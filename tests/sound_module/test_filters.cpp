...
TEST_CASE("TimestretchFilter: размер заголовка пересчитан", "[timestretch]")
{
    ...
    TEST_CASE("TimestretchFilter: пустой сигнал не меняется",
              "[timestretch][edge]")
    {
        Waveform wf = makeWaveform({});
        TimestretchFilter f{2.0};
              ...
              "[sin][generator]")  // NOLINT
    {
        ...
        REQUIRE(s.size() == 5);
        REQUIRE(
            std::all_of(s.begin(), s.end(), [](int16_t v) { return v == 0; }));
    }
    ...
    TEST_CASE(
        "AmGenerator: amplitude 0 -> нулевой сигнал нужной длины",  // NOLINT
        "[am][generator]")
    {
        Waveform wf = makeWaveform({}, 1000);
    ...
    TEST_CASE(
        "FmGenerator: amplitude 0 -> нулевой сигнал нужной длины",  // NOLINT
        "[fm][generator]")
    {
        Waveform wf = makeWaveform({}, 1000);
